...
static void do_viasend(const char *input_str);
static void do_viarecv(void);
...
static bool monitor_parse_command(const char *line)
{
    ...
    else if (strcmp(cmd, "viasend") == 0) {
        do_viasend(args);
        return false;
    }
    else if (strcmp(cmd, "viarecv") == 0) {
        do_viarecv();
        return false;
    }
    else
...
static void print_help(void)
{
    ...
    printf("  viasend <texto>         - Alimenta buffer serial do VIA\n");
    printf("  viarecv                 - Lê próximo caractere da serial do VIA\n");
    printf("\nExamples:\n");
...
static void do_memdump(uint16_t start, int count)
{
    ...
    uint16_t end             = (uint16_t)(start + count - 1);
    ...
    for (int i = 0; i < count; i += bytes_per_line)
    {
        ...
        {
            uint16_t addr = (uint16_t)(start + i + b);
            buffer[b]     = bus_read_memory(&g_bus, addr);
        }

        print_ascii_table_line((uint16_t)(start + i), buffer, line_len);
    }
...
static void do_stackdump(int count)
{
    ...
    if (start_addr > end_addr || count <= 0)
    {
        printf("[STACK] Nothing to dump. SP=0x%02X\n", cpu->sp);
        return;
    }

    /* Ensure we don't read beyond stack boundary */
    int max_count = (int)(end_addr - start_addr + 1);
    if (count > max_count)
        count = max_count;

    uint16_t final_addr = (uint16_t)(start_addr + count - 1);
    ...
    for (int i = 0; i < count; i += bytes_per_line)
    {
        ...
        for (int b = 0; b < line_len; b++)
        {
            uint16_t addr = (uint16_t)(start_addr + i + b);
            buffer[b]     = bus_read_memory(&g_bus, addr);
        }

        print_ascii_table_line((uint16_t)(start_addr + i), buffer, line_len);
    }
...
static void do_viasend(const char *input_str) {
    if (!g_bus.via) {
        printf("[ERROR] VIA não inicializado.\n");
        return;
    }
    via_serial_feed(g_bus.via, input_str);
    printf("[OK] Buffer serial do VIA alimentado: '%s'\n", input_str);
}
...
static void do_viarecv(void) {
    ...
    if (g_bus.via->serial_in_head != g_bus.via->serial_in_tail) {
        uint8_t c = via_read(g_bus.via, VIA_BASE_ADDRESS + VIA_REG_ORA);
        printf("[VIA Serial IN] 0x%02X ('%c')\n", c, (c >= 32 && c < 127) ? c : '.');
    } else {
        printf("[VIA Serial IN] Nenhum dado disponível.\n");
    }
}
