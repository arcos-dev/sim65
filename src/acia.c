 ...
 *
 * This so            // Check if TX buffer has space
            uint16_t next_head = (uint16_t)((acia->tx_head + 1) % ACIA_TX_BUFFER_SIZE);
            ...
            if (next_head != acia->tx_tail)
            {
                ...
                acia->status_reg &= (uint8_t)(~ACIA_STATUS_TX_READY);ovides the implementation for the ACIA 6550/6551
    ...
    switch (offset)
    {
        ...
        case ACIA_REG_DATA_RX:
        {
            if (acia->rx_ready)
            {
                ...
                // Update RX tail
                acia->rx_tail = (uint16_t)((acia->rx_tail + 1) % ACIA_RX_BUFFER_SIZE);

                // Check if RX buffer is now empty
                if (acia->rx_tail == acia->rx_head)
                {
                    acia->rx_ready = false;
                    acia->status_reg &= (uint8_t)(~ACIA_STATUS_RX_READY);
                }
    ...
    switch (offset)
    {
        case ACIA_REG_DATA_TX:
        {
            ...
            if (next_head != acia->tx_tail)
            {
                ...
                // Update status register
                acia->status_reg &= (uint8_t)(~ACIA_STATUS_TX_READY);
...
void acia_process_tx(Acia6550 *acia)
{
    ...
    while (acia->tx_tail != acia->tx_head)
    {
        ...
        // Update TX tail
        acia->tx_tail = (uint16_t)((acia->tx_tail + 1) % ACIA_TX_BUFFER_SIZE);
...
void acia_provide_input(Acia6550 *acia, const char *data)
{
    ...
    while (*data)
    {
        ...
        uint16_t next_head = (uint16_t)((acia->rx_head + 1) % ACIA_RX_BUFFER_SIZE);
