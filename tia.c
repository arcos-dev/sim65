...
void tia_cycle(TIA *tia)
{
    ...
    tia->color_clock++;
    if (tia->color_clock >= TIA_CYCLES_PER_SCANLINE)
    {
        tia->color_clock = 0;
        tia->scanline++;

        /* If we passed the last scanline, that's the end of the frame. */
        if (tia->scanline >= TIA_SCANLINES_PER_FRAME)
        {
            tia->scanline   = 0;
            tia->frame_done = true;
            tia->frame_count++;
...
void tia_render_pixel(TIA *tia)
{
    ...
    if (tia->vblank)
        return;
    if (y >= TIA_SCREEN_HEIGHT)
        return;
    if (x >= TIA_SCREEN_WIDTH)
        return;
