...
static inline void push16(uint16_t value)
{
    ...
    bus_write_memory(cpu->bus, STACK_BASE + cpu->sp, (value >> 8) & 0xFF);
...
static void am_relative(void)
{
    ...
    cpu->effective_addr = branch_pc + 1 + offset;
}
...
static void op_adc(void)
{
    cpu->penalty_opcode = 1;
    uint8_t operand = get_operand(opcode_table[cpu->current_opcode].addr_mode);
    uint16_t result = cpu->a + operand + cpu->flag_c;
    ...
    {
        /* Decimal mode (BCD) */
        result = (cpu->a & 0x0F) + (operand & 0x0F) + cpu->flag_c;

        if (result >= 0x0A)
            result = ((result + 0x06) & 0x0F) + 0x10;

        result += (cpu->a & 0xF0) + (operand & 0xF0);
...
static void op_sbc(void)
{
    ...
    {
        /* BCD subtraction */
        uint16_t al = (cpu->a & 0x0F) - ((operand ^ 0xFF) & 0x0F) + old_c - 1;

        if ((int16_t) al < 0)
            al = ((al - 0x06) & 0x0F) - 0x10;

        result = (cpu->a & 0xF0) - ((operand ^ 0xFF) & 0xF0) + al;
...
static void op_sha(void)
{
    put_operand(opcode_table[cpu->current_opcode].addr_mode,
                cpu->a & cpu->x & (((cpu->effective_addr >> 8) + 1) & 0xFF));
}

/* Unofficial: SHX (store X & (high-byte+1)) with weird wrap */
static void op_shx(void)
{
    uint8_t value = cpu->x & ((((cpu->effective_addr - cpu->y)) >> 8) + 1);
...
static void op_shy(void)
{
    uint8_t value = cpu->y & ((((cpu->effective_addr - cpu->x)) >> 8) + 1);
...
static void op_tas(void)
{
    cpu->sp = cpu->a & cpu->x;
    put_operand(opcode_table[cpu->current_opcode].addr_mode,
                cpu->sp & (((cpu->effective_addr >> 8) + 1) & 0xFF));
}
